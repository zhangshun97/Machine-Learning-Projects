...
def trick_content_img(trick, content_img):
    if trick == 1:
        ...
        # 浮雕滤波
        content_img = content_img.filter(ImageFilter.EMBOSS)
        # content_img = content_img.filter(ImageFilter.CONTOUR)
    elif trick == 8:
        # find edges
        content_img = content_img.filter(ImageFilter.FIND_EDGES)
        # content_img = content_img.filter(ImageFilter.CONTOUR)
    elif trick == 9:
        # sharpen, more clear
        content_img = content_img.filter(ImageFilter.SHARPEN)
        content_img = content_img.filter(ImageFilter.CONTOUR)
    elif trick == 10:
        # black and white
        content_img = content_img.convert('1')
        ...
        # 浮雕滤波
        content_img = content_img.filter(ImageFilter.CONTOUR)
        loader_r = transforms.ToTensor()
        content_img = loader_r(content_img)
        # print(torch.max(c))
        # unloader(c).show()
        content_img[content_img < 0.9] = 0
        # unloader(c).show()
...
def style_transfer(style_file, content_file, trick, num_steps,
                   style_weight=1000, content_weight=1, out_dir='./'):
    ...
    loader = transforms.Compose([
        transforms.CenterCrop((content_img.size[1], content_img.size[0])),
        transforms.ToTensor(),
    ])
    ...
    imsave(input_img.data, path=os.path.join(out_dir, 'Input_content_{}.png'.format(trick)))
    imsave(style_img.data, path=os.path.join(out_dir, 'Input_style_{}.png'.format(trick)))

    output = run_style_transfer(cnn,
                                content_img=trick_img,
                                style_img=style_img,
                                input_img=contour_img,  # for initialization
                                num_steps=num_steps,
                                style_weight=style_weight,
                                content_weight=content_weight,
                                )
    imsave(output, path=os.path.join(out_dir, 'output.png'))
...
tricks = {
    ...
    'emboss': 7,  # ImageFilter.EMBOSS为浮雕滤波
...
parser.add_argument('--style-file', type=str, default='./demoInput/style.png', metavar='S',
                    help='input style image to transfer')
parser.add_argument('--content-file', type=str, default='./demoInput/content.jpg', metavar='C',
                    help='input content image to transfer')
parser.add_argument('--trick', type=str, default='none', metavar='T',
                    help='what trick to use, including: {}'.format(tricks.keys()))
parser.add_argument('--num-steps', type=int, default=300, metavar='N',
                    help='Number of training steps')
parser.add_argument('--style-weight', type=int, default=1000, metavar='SW',
                    help='style image weight (INT), default as 1000')
parser.add_argument('--content-weight', type=int, default=4, metavar='CW',
                    help='content image weight (INT), default as 4')
parser.add_argument('--out-dir', type=str, default='./demoOutput', metavar='O',
                    help='output file directory')
args = parser.parse_args()
style_transfer(style_file=args.style_file,
