...
def run_logistic_regression():
    ...
    plt.plot(list(range(hyperparameters['num_iterations'])), ce_train, color='blue')
    plt.plot(list(range(hyperparameters['num_iterations'])), ce_valid, color='green')
    plt.title('logistic regression(learning rate = {})'.format(hyperparameters['learning_rate']))
    plt.xlabel('the number of iteration')
    plt.ylabel('cross entropy')
    plt.legend(['train(%1.2f)' % ce_train[-1],
                'valid(%1.2f)' % ce_valid[-1]])
    plt.show()
